...
export default function ComparisonChart({ yourPrice, competitorPrice }) {
  const data = [
    ...
    { 
      ...
      fill: "#ef4444" // Warning Red
    }
  ];

  // Custom gorgeous tooltip for hover events
  ...
  return (
    <div className="bg-white dark:bg-[#1f1f1f] p-6 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 mb-6 transition-all duration-300 hover:shadow-md">
      ...
      {/* Increased height so vertical bars look impressive but not overwhelming */}
      <div className="w-full h-80">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart 
            data={data} 
            margin={{ top: 20, right: 30, left: 20, bottom: 5 }}
            barSize={120} // Limits bar width so they aren't ridiculously fat
          >
            ...
            {/* The Tooltip gives the beautiful hover interactions */}
            <Tooltip 
              content={<CustomTooltip />}
              cursor={{fill: 'rgba(200, 200, 200, 0.1)'}} 
            />
            
            {/* animationDuration makes the bars dramatically shoot up from the starting line */}
            <Bar 
              dataKey="price" 
              radius={[8, 8, 0, 0]} 
