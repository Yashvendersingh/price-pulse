...
export default function ProductTable({ products }) {
  ...
  // Extract unique brands dynamically from the first word of the product name
  const uniqueBrands = useMemo(() => {
    if (!products) return ["All Brands"];
    const brands = new Set(
      products.map(p => {
        const firstWord = p.product_name.trim().split(" ")[0];
        // Handle special cases or clean up punctuation if any exists
  ...
  // Filter products by selected brand AND selected status
  const filteredProducts = useMemo(() => {
    ...
    // 2. Filter by Status (Increase vs Decrease)
